...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, CommandNotFound):
        ...
        if ctx.guild:  # chỉ xét trong server
            try:
                data = load_data()
                allowed = get_guild_channels(data, ctx.guild.id)  # tập kênh đã setbot
            except Exception:
                allowed = set()
            # Chưa setbot hoặc kênh hiện tại không thuộc danh sách -> im lặng, không báo gì
            if not allowed or (ctx.channel.id not in allowed):
                return
        await ctx.reply("❓ Lệnh không tồn tại. Dùng `olenh` để xem danh sách.", mention_author=False)
        return
    ...
    # Các lỗi khác → ném lên để dễ debug log
    raise error
...
async def cmd_onhanvat(ctx, member: discord.Member=None):
    ...
    # Ảnh nhân vật riêng
    file = await file_from_url_cached("https://sv2.anhsieuviet.com/2025/10/29/nhanvat-min.png", "nhanvat.png")
...
async def global_channel_check(ctx: commands.Context):
    ...
    # Không phải lệnh thật của bot này
    if ctx.command is None:
        return True

    # Lấy toàn bộ tên có thể của lệnh: name + alias
    cmd_names = {ctx.command.name.lower(), *[a.lower() for a in getattr(ctx.command, "aliases", [])]}

    # Cho phép các lệnh quản trị/tiện ích
    if cmd_names & ADMIN_WHITELIST:
        return True

    # Nếu là lệnh gameplay → chỉ cho phép ở kênh đã setbot
    if cmd_names & GAMEPLAY_REQUIRE:
        ...
        if not allowed or (ctx.channel.id not in allowed):
            msg = (
                ...
                "Yêu cầu Admin Discord dùng **`osetbot`** để chỉ định kênh chạy BOT cho server này."
            )
            try:
                await ctx.reply(msg, mention_author=False)
            except Exception:
