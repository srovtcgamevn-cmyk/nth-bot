...
async def cmd_setlink(ctx: commands.Context, invite_url: str, *roles: discord.Role):
    if not is_owner(ctx.author.id):
        ...
        return
    if not roles:
        await ctx.reply("❌ Phải tag ít nhất 1 role.")
        return
    code = invite_url.strip().split("/")[-1]
    data = load_json(BUFF_FILE, {"guilds": {}})
    gid = str(ctx.guild.id)
    if gid not in data["guilds"]:
        data["guilds"][gid] = {"buff_enabled": True, "links": {}}
    data["guilds"][gid]["links"][code] = {
        "role_ids": [r.id for r in roles],
        "active": True
    }
    save_json(BUFF_FILE, data)
    await ctx.reply(f"✅ Gán link `{code}` với role: {' '.join(r.mention for r in roles)}")
...
async def cmd_xemlink(ctx: commands.Context):
    if not is_owner(ctx.author.id):
        ...
        return
    data = load_json(BUFF_FILE, {"guilds": {}})
    gconf = data["guilds"].get(str(ctx.guild.id))
    if not gconf:
        await ctx.reply("📭 Chưa có link buff.")
        return
    lines = [f"Buff: {'BẬT' if gconf.get('buff_enabled', True) else 'TẮT'}"]
    for code, conf in gconf.get("links", {}).items():
        lines.append(f"- {code}: {conf.get('role_ids', [])} ({'ON' if conf.get('active',True) else 'OFF'})")
    await ctx.reply("\n".join(lines))
...
async def cmd_xoalink(ctx: commands.Context, invite_url: str):
    ...
    data = load_json(BUFF_FILE, {"guilds": {}})
    gconf = data["guilds"].get(str(ctx.guild.id))
    if not gconf or code not in gconf.get("links", {}):
        await ctx.reply("❌ Link này chưa set.")
        return
    gconf["links"][code]["active"] = False
    data["guilds"][str(ctx.guild.id)] = gconf
    save_json(BUFF_FILE, data)
    await ctx.reply("✅ Đã tắt link.")
...
async def cmd_tatbuff(ctx: commands.Context):
    ...
    gid = str(ctx.guild.id)
    if gid not in data["guilds"]:
        data["guilds"][gid] = {"buff_enabled": False, "links": {}}
    else:
